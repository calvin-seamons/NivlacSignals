...
class VolatilityEstimator(object):
    ...
    def cones(self, windows=[30, 60, 90, 120], quantiles=[0.25, 0.75]):
        ...
        cones.set_yticks(locs)  # Set ticks before setting labels
        ...
        # set and format the y-axis labels
        locs = box.get_yticks()
        box.set_yticks(locs)  # Set ticks before setting labels
    ...
    def term_sheet(
        ...
        fn = os.path.join(term_sheets_dir, filename)
        pp = PdfPages(fn)
        ...
        print(f'Term sheet saved to: {fn}')
